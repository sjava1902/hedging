...
engine.step(1)
print("accrued_interest after 1d:", engine.accrued_interest)
...
# старая кривая (тенор-by-тенор)
gc = GCurve(portfolio.T0, base)
# новая кривая NS (калибруется по тем же точкам)
gns = NSCurve(portfolio.T0, base_points=base)
print("GCurve:", GCurve(portfolio.T0, base).snapshot())
print("NSCurve:", gns.snapshot())

# если хочешь прогнать движок на NS вместо старой кривой:

...
class HedgeEngine:
    ...
    def __init__(self, portfolio: Portfolio, gcurve: GCurve | None = None):
        ...
        self.t_curr = self.t0
        self.bank_account = 0.0          # счёт с овернайтом
        self.accrued_interest = 0.0      # накопленные купоны (квартально выплачиваемые)
    ...
    def step(self, days: int = 1):
        for _ in range(days):
            self._accrue_one_day()            # кредиты/депозиты
            self._accrue_swaps_one_day()      # свопы
            self._age_and_rollover()          # бумаги
            self._age_swaps_and_rollover()    # свопы
    ...
    def _accrue_one_day(self):
        df = self.portfolio.get_portfolio()

        # дневная ставка по инструменту = годовая / 365
        daily_rates = df["rate"] / 365.0

        # проценты по кредитам (мы получаем)
        loan_mask = df["type"] == "loan"
        interest_loans = (df.loc[loan_mask, "volume"] * daily_rates[loan_mask]).sum()

        # проценты по депозитам (УТОЧНЕНИЕ знака см. ниже)
        dep_mask = df["type"] == "deposit"
        interest_deps = (df.loc[dep_mask, "volume"] * daily_rates[dep_mask]).sum()

        # print("DEBUG daily: loans=", interest_loans, "deps=", interest_deps),

        # накапливаем купоны (квартально спишем/зачислим на bank_account)
        # ЕСЛИ депозиты — наш долг клиентам, то их проценты вычитаем (минус).
        # ЕСЛИ депозиты — это наши размещения, то плюсуем (плюс).
        self.accrued_interest += interest_loans  # плюс всегда
        self.accrued_interest += (-interest_deps)  # ← ВРЕМЕННОЕ решение, см. вопрос ниже

        # овернайт на остаток счёта
        r_on = self.gcurve.r_overnight()
        self.bank_account *= (1.0 + r_on / 365.0)
    ...
    def _quarterly_settle(self):
        ...
        if self.days_since_quarter_start >= QUARTER_LEN_DAYS:
            if hasattr(self, "optimizer") and callable(getattr(self.optimizer, "rebalance_once", None)):
                ...
                # применим: если x_T > 0 — докупаем receive_fixed, если < 0 — pay_fixed
                if decision.x_6 != 0:
    ...
    def _age_and_rollover(self):
        df = self.portfolio.get_portfolio()

        # 1) уменьшаем остаточный срок
        df.loc[:, "remaining_months"] = df["remaining_months"] - (1.0 / DAYS_PER_MONTH)

        # 2) пролонгация тех, кто погасился
        matured_idx = df.index[df["remaining_months"] <= 0].tolist()
        if matured_idx:
            for idx in matured_idx:
                ...
                df.at[idx, "start_date"] = self.t_curr
                df.at[idx, "maturity_date"] = self.t_curr + relativedelta(months=term)
                df.at[idx, "remaining_months"] = float(term)
                df.at[idx, "rate"] = new_rate
                df.at[idx, "type"] = typ

        # 3) сохранить изменения
        self.portfolio.set_portfolio(df)
    ...
    def add_swap(self, direction: str, term_months: int, notional: float):
        ...
        row = {
            ...
            "fixed_rate": fixed,
            "float_rate_q": flt,           # фиксируется на квартал, пересчёт при квартальной оплате
            "start_date": start,
            "maturity_date": mat,
        }
    ...
    def _accrue_swaps_one_day(self):
        if self.swaps.empty:
            # даже если нет свопов — овернайт на счёт всё равно капает
            r_on = self.gcurve.r_overnight()
            self.swap_account *= (1.0 + r_on / 365.0)
            return

        # дневная «начисленная разница купонов» по всем свопам (фикс/флоат)
        # Аппроксимация: в течение квартала плавающая нога = фиксированная на flt (reset в квартал)
        # Купоны платятся раз в квартал, но мы копим ежедневно
        daily_net = 0.0
        for _, s in self.swaps.iterrows():
            fixed_leg = s["notional"] * s["fixed_rate"] / 365.0
            float_leg = s["notional"] * s["float_rate_q"] / 365.0

            if s["direction"] == "pay_fixed":
                # платим фикс, получаем плавающую
                daily_net += (float_leg - fixed_leg)
            else:  # receive_fixed
                daily_net += (fixed_leg - float_leg)
